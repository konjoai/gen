...
| `state_expansion` | 2026-06-24 | promoted | P/~/~/~ | first both-axes row: recall far above ssm through K<=16 (K=8 Wilcoxon p=4.6e-5) AND state-tracking far above attention (parity/dyck=1.0; p=1.8e-6). Error-correcting delta write + beta in (0,2) negative-eigenvalue lever. K=32 recall ceiling at default d_key=32 (0.151) is a capacity knob, not a wall: d_key=64 recovers it to 0.991 (Gate-4 scaling). Clears the pre-registered corner bar. | [doc](docs/screens/state_expansion.md) |
